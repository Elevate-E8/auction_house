...
<body>
  ...
  <header>
    ...
    if (isLoggedIn) {
        std::cout << R"(  <a href='index.cgi'>Home</a>
    ...
    if (!isLoggedIn) {
        std::cout << R"(    <a class='btn primary' href='register.cgi'>Create an account</a>
            <a class='btn ghost' href='login.cgi'>Log in</a>
)";
    }
    else {
        std::cout << R"(    <a class='btn primary' href='list_auctions.cgi'>Browse auctions</a>
